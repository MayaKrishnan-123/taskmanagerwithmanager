...
def add_task():
    ...
    if form.validate_on_submit():
        task = Task(name=form.name.data,
                                description=form.description.data,status=form.status.data,importance=form.importance.data,towhom=form.towhom.data,is_assigner=form.is_assigner.data)
        try:
...
def list_task():
    ...
    check_manager()
    tasks = Task.query.filter_by(is_assigner='True')
    return render_template('manager/task/task_showall.html',
                           tasks=tasks, title="Tasks")

@manager.route('/taskmanager/toassign',methods=['GET','POST'])
@login_required
def to_assign():
    check_manager()
    tasks = Task.query.all()
    return render_template('manager/task/task_toassign.html',tasks=tasks,title="Tasks")
